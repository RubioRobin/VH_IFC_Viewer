...
router.get('/:token', async (req, res) => {
    ...
    console.log(`[SHARE] Access request for token: ${token}`);

    try {
        // 1. Try New Share (Token)
        const share = await db.getShareByToken(token);
        console.log(`[SHARE] Share lookup result:`, share ? 'Found' : 'Not Found');
        ...
        if (share) {
            ...
            await db.logActivity(project.id, 'Guest', 'scan', `Share Scan: ${model.name}`);
        ...
        console.log(`[SHARE] PublicLink lookup result:`, publicLink ? 'Found' : 'Not Found', publicLink);
