...
if (params.model) {
  // Dynamic loading
  try {
    const response = await fetch(`/models/${params.model}`);
    if (!response.ok) throw new Error(`Could not fetch model: ${params.model}`);
...
// Optionally load a default model if no param?
// For now, only load if explicitly asked or upload mechanism (not part of this task)
...
fragments.list.onItemSet.add(async ({ value: model }) => {
  ...
  if (elementId) {
    try {
      // Find model containing this element - simplest is to check all models or just the loaded one?
      // Since onItemSet triggers for EACH model loaded, we just check this model.
      ...
      if (modelIdMap && Object.keys(modelIdMap).length > 0) {
        ...
        try {
          ...
          console.warn("Error zooming to element:", bboxError);
        }
      }
    } catch (e) {
      console.warn("Error highlighting element from URL:", e);
    }
  }
});
