...
function renderModelTree(
  ...
  for (const entry of entries) {
    ...
    modelIcon.style.cssText = "color:#4f46e5;flex-shrink:0;";
    ...
    modelEyeBtn.style.cssText = `
      ...
      color:${entry.allVisible ? "#4f46e5" : "#9ca3af"};
    ...
    if (visibleStoreys.length === 0) {
      ...
      for (const storey of visibleStoreys) {
        ...
        eyeBtn.style.cssText = `background:none;border:none;cursor:pointer;padding:2px;color:${storey.visible ? "#4f46e5" : "#9ca3af"};display:flex;align-items:center;`;
